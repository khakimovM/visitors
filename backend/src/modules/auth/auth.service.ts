...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    ...
    await this.redisService.set(
      ...
      JSON.stringify({ ...registerDto, password: hashedPassword }),
      300
    );
  ...
  async verifyRegistrationOTP(verifyOtpDto: VerifyOtpDto) {
    ...
    const user = await this.userService.create({
      email,
      username,
      password, // already hashed at registration step
    });
  ...
  async forgotPassword(email: string) {
    ...
    const genericResponse = {
      message: "Agar bu email ro'yxatdan o'tgan bo'lsa, OTP yuborildi",
    };
